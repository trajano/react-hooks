...
describe("Clock update", () => {
  it("should work when starting at zero seconds", async () => {
    ...
    expect(screen.getByTestId("test").textContent).toEqual('2020-01-01T00:00:00.000Z');
    ...
    await act(() => jest.advanceTimersByTime(500));
    expect(screen.getByTestId("test").textContent).toEqual('2020-01-01T00:00:01.000Z');
    await act(() => jest.advanceTimersByTime(500));
    expect(screen.getByTestId("test").textContent).toEqual('2020-01-01T00:00:01.000Z');

    await act(() => jest.advanceTimersByTime(500));
    expect(screen.getByTestId("test").textContent).toEqual('2020-01-01T00:00:02.000Z');
    unmount();
  });
  ...
  it("should work when starting at non-zero seconds", async () => {
    ...
    const { unmount } = render(<PerSecondClock />)
    expect(screen.getByTestId("test").textContent).toEqual('2020-01-01T00:00:00.500Z');
    expect(renderCallback).toBeCalledTimes(1);
  ...
  it("should work when starting at non-zero seconds with day interval", async () => {
    ...
    expect(screen.getByTestId("test").textContent).toEqual('2020-01-01T12:00:00.500Z');
    expect(renderCallback).toBeCalledTimes(1);
    ...
    expect(screen.getByTestId("test").textContent).toEqual('2020-01-02T00:00:00.000Z');

    await act(() => jest.advanceTimersByTime(24 * 60 * 60 * 1000));
    expect(screen.getByTestId("test").textContent).toEqual('2020-01-03T00:00:00.000Z');
    unmount();
  });
