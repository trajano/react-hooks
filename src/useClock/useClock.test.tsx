...
describe("Clock update", () => {
  it("should work when starting at zero seconds", async () => {
    ...
    expect(screen.getByTestId("test").textContent).toEqual(new Date('2020-01-01T00:00:00Z').getTime().toString());
    ...
    await waitFor(() => {
      expect(screen.getByTestId("test").textContent).toEqual(new Date('2020-01-01T00:01:00Z').getTime().toString());
    });
    // clear off duplicate calls
    renderCallback.mock.calls = renderCallback.mock.calls.reduce((prev, next) => isEqual(prev[prev.length - 1], next) ? prev : [...prev, next], []);
    ...
    expect(screen.getByTestId("test").textContent).toEqual(new Date('2020-01-01T00:02:00Z').getTime().toString());
    unmount();
  })
  it("should work when not starting at zero seconds", async () => {
    ...
    expect(screen.getByTestId("test").textContent).toEqual(new Date('2020-01-01T00:00:12.878Z').getTime().toString());
